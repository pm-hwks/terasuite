...
#!/bin/bash
# Usage : testdfsio-read.sh --size=<10|100|1000|10000 (MB)> --nooffiles=10 --specs='3x R4XL(7-vcpu,32GB-RAM,250GB-EBS)' --comments='initial run'>
...
RESULTSFILE="$LOGDIR/testdfsio-read_results_$DATE"
...
OP=$(echo "***METRICS101*** |  testdfsio-read.sh | $(printf "%8s" $SIZE) | $(printf "%4s" $NOF) | $DURATION | $(printf "%6s" $secs) | specs: $(printf '%-20s' "${SPECS}") | comments: $(printf "%-50s" "${COMMENTS}")") 
